...
function getCurrentDuty() {
  try {
    const raw = sessionStorage.getItem(STORAGE_CURRENT_DUTY);
    return raw ? JSON.parse(raw) : null;
  } catch {
    return null;
  }
...
function setCurrentDuty(data) {
  if (data) {
    sessionStorage.setItem(STORAGE_CURRENT_DUTY, JSON.stringify(data));
  } else {
    sessionStorage.removeItem(STORAGE_CURRENT_DUTY);
  }
}
